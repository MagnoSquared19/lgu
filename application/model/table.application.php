...
{	
	...
	function newAccount($d)
	{
		...
		$bday = $fd['bday_year']."-".$fd['bday_month']."-".$fd['bday_day'];

		$query = $db->db_insert_id("INSERT INTO `residents`(fn,mn,ln,suffix,gender,bday,email,cp,civil_status,birth_place,occupation,nationality_id,pwd,street_address,brgy_id,region_id,province_id,city_id,id_philhealth,id_pwd,un,pw) VALUES(
			'".mysqli_real_escape_string($db_con, $fd['fn'])."',
			'".mysqli_real_escape_string($db_con, $fd['mn'])."',
			'".mysqli_real_escape_string($db_con, $fd['ln'])."',
			'".mysqli_real_escape_string($db_con, $fd['suffix'])."',
			'".mysqli_real_escape_string($db_con, $fd['gender'])."',
			'".mysqli_real_escape_string($db_con, $bday)."',
			...
			'".mysqli_real_escape_string($db_con, $fd['region'])."',
			'".mysqli_real_escape_string($db_con, $fd['province'])."',
			'".mysqli_real_escape_string($db_con, $fd['city'])."',
	...
	if($_POST['gt'] == '100-1')
	{
		...
		function covid_exec($id)
		{
			...
			$q30= [];
			array_push($q30, (!isset($fd['q30'])?"0":"1"));
			array_push($q30, (!isset($fd['q31'])?"0":"1"));
			array_push($q30, (!isset($fd['q32'])?"0":"1"));
			array_push($q30, (!isset($fd['q33'])?"0":"1"));
			array_push($q30, (!isset($fd['q34'])?"0":"1"));
			array_push($q30, (!isset($fd['q35'])?"0":"1"));
			array_push($q30, (!isset($fd['q36'])?"0":"1"));

			$q10_date = (($fd['q10_year']=="")?"0000":$fd['q10_year'])."-".(($fd['q10_month']=="")?"00":$fd['q10_month'])."-".(($fd['q10_day']=="")?"00":"");

			/*$q10_date = (($fd['q10_year']=="")?"0000":$fd['q10_year'])."-".(($fd['q10_month']=="")?"00":$fd['q10_month'])."-".(($fd['q10_day']=="")?"00":$fd['q10_day']);*/

			$query = $db->db_insert_id("INSERT INTO covid_immunization(`trans_no`,`resi_id`,`dated`,`company_name`,`company_phone`,`company_address`,`q0`,`q1`,`q2`,`q3`,`q4`,`q5`,`q6`,`q7`,`q8`,`q9`,`q10`,`q10_date`,`q10_class`,`q20`,`q30`,`q30_other`,`remarks`,`category`,`id_type`,`id_type_no`,`emp_type`,`emp_icc`) VALUES(
				...
				'".mysqli_real_escape_string($db_con, $q10_date)."',
				'".mysqli_real_escape_string($db_con, $fd['q10_class'])."',				
				'".mysqli_real_escape_string($db_con, implode(",", $q20))."',
				'".mysqli_real_escape_string($db_con, implode(",", $q30))."',
				'".mysqli_real_escape_string($db_con, $fd['q30_other'])."',	
