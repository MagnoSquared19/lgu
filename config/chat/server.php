...
{	
	...
	if(in_array($socket, $changed)) 				//check for new socket	
	{
		...
		socket_getpeername($socket_new, $ip); 		//get ip address of connected socket  		//parse_url($header)
		$response = mask(json_encode(array('type'=>'system', 'data'=>$ip.' connected - id:'.$header." - "))); //prepare json data
