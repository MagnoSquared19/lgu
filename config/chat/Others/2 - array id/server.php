...
$id=0;
$clients = array('id'=>($id), 'src'=>$socket);
...
{	
	...
	socket_select($changed['src'], $null, $null, 0, 10);	//returns the socket resources in $changed array // waiting for socket to change status
	//socket_select($changed, $null, $null, 0, 10);
	
	if(in_array($socket, $changed)) 				//check for new socket	
	{
		$socket_new = socket_accept($socket); 						//accept new socket
		$clients[] = array('id'=>($id+=1), 'src'=>$socket_new); 	//add socket to client array
		
		$header = socket_read($socket_new, 1024); 	//read data sent by the socket
		perform_handshaking($header, $socket_new, $host, $port); //perform websocket handshake
		
		socket_getpeername($socket_new, $ip); 		//get ip address of connected socket
		$response = mask(json_encode(array('type'=>'system', 'data'=>$ip.' connected'.'-'.$id))); //prepare json data
		//$response = mask(json_encode(array('type'=>'system', 'data'=>$ip.' connected'.'-'.$clients[$id]['id']))); //prepare json data
		...
		socket_getpeername($socket_new, $ip); 		//get ip address of connected socket
		$response = mask(json_encode(array('type'=>'system', 'data'=>$ip.' connected'))); //prepare json data
		send_message($response); 					//notify all users about new connection		
	...
	{			
		while(socket_recv($changed_socket, $buf, 10240, 0) >= 1)	//check for any incomming data. 1024 limit size of data
		{
			...
			$response_text = mask(json_encode(array('type'=>$ftr_, 'data'=>$user_data."-".$clients[1])));
...
function send_message($msg)
{
	...
	{
		@socket_write($clients[$i]['src'],$msg,strlen($msg));
	}
	/*foreach($clients['src'] as $changed_socket)
	{	
