...
$(document).ready(function(e)
{
	if($GET('value') !== "")
	{ 
		if($GET('value').includes("-"))
		{
			var ol_arr = ["bc","rc","ci","ja","jx"];
			var isRes= $GET('value').split("-");
			if(ol_arr.includes(isRes[0].toLowerCase())){ $(".searchFilter_sel").val("1"); }
		}
		$(".pageSearch_ipt").val($GET('value')); setSearch_exec();
	}
	$(".pageSearch_form").submit(function(e){ e.preventDefault(); setSearch_exec(); });

});

function setSearch_exec()
{
	//alert($(".pageSearch_ipt").val());
	var isEmpty = true;
	$.post('embed/model/table.search.php',{token:"0",fb:"27",valid:"0-0",like:$(".pageSearch_ipt").val()},function(json, status, xhr)
	{		
		$.each(json,function(i, v)
		{
			...
			{
				isEmpty = false;
				var tmpUri= (v.uri=="1")?"events-post.php":(v.uri=="2")?"press-release-post.php":"#";
				...
				'<div class="pagelet-post" id="eventPost'+v.id+'"><div class="page-load-pagelet-in">'+
					'<div class="pagelet-body-in" >'+
						...
						'<div class="poster-post-label" '+((v.dated)?'':'hidden')+'><i class="fa fa-calendar poster-label-ico"></i>	<p>'+dateToString(v.dated)+'</p></div>'+
						'<pre class="poster-post-body rich-paragraph">'+obj.text().substring(0, 160)+' ... <a href="'+tmpUri+'?id='+v.id+'" '+((v.uri)?'':'hidden')+'>Read More</a></pre>'+
					...
					'</div>'+
				'</div></div>'; 
				$(".searchLoad_wrap").append(tPostResult);
			}
			if(v.count){ $(".searchNum_rows").html("<b>"+v.count+"</b> Result"+((parseInt(v.count)>1)?"s":"")); }
		});
	},"json").done(function(e)
	{
		$(".searchLoad_wrap .ldgPost_wrap").remove(); if(isEmpty)
		{ 
			$(".searchLoad_wrap").append('<div class="evResult_val ui-post-no-result"><img src="images/icons/no-result-found.png" /><p>No Record Found</p></div>'); $(".searchNum_rows").text("0 Result");
		}
		...
		*/global_events(); setUrlVal("reset");

	}).fail(function(xhr, status, err){ $(".searchLoad_wrap .ldgPost_wrap").remove(); $(".searchNum_rows").text("0 Result"); alert_("Error in: "+xhr.responseText+"<br/><br/>Status: ("+status+")<br/>"+err,"Error Message");	});
	$(".searchLoad_wrap").empty().append(ldgPost); $(".searchNum_rows").html('<i class="italic">waiting...</i>');
}
