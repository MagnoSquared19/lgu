...
function gUploader($id="",$nme="",$dir_="",$ipt="",$ext="",$w="",$h="",$replace="")
{
	...
	if(!is_dir($loc)){	mkdir($loc, 0755);	}
	if($dir!="")
	{	
		$tmpDir=explode("/", $sDir);
		if(count($tmpDir)==1){	if(!is_dir($dir)){mkdir($dir, 0755);}	}
		else
		{
			$tVal = "";
	...
	for($i=0;$i<count($_FILES[$tFile]["name"]);$i++)
	{
		...
		$fExt = ($ext!="")?$ext:end($Nme_arr); 
		$fName= rtrim($tmpName,".".end($Nme_arr));
...
function resize_imageX($src,$dest,$toWidth,$toHeight,$ext,$options=array()) 
{
	
	$frame = imagecreatefrompng($src);
	//$thumb = imagecreatefromjpeg('../files/userX/27/thumbnail.jpg');
	$width = imagesx($toWidth); 
	$height = imagesy($toHeight); 
...
function resize_imageXX($src,$dest,$toWidth,$toHeight,$ext,$options=array()) 
{	
	if(!file_exists($src)){ echo "$src file does not exist";	die(); }
	$ext = strtolower($ext);
    if ($ext == "gif"){	$img = imagecreatefromgif($src);	} 
	if($ext =="png")  {	$img = imagecreatefrompng($src);	} 
	else{	$img = imagecreatefromjpeg($src);	}
	...
	imagecopyresampled($imageResized, $img , 0 , 0 , 0 , 0 , $new_width , $new_height , $width , $height);
	imagealphablending($imageResized, false);	
	imagesavealpha($imageResized, true);*/
	if(!($imageResized = imagecreatetruecolor($new_width, $new_height))){ echo "Could't create new image resource width: $new_width, height: $new_height";die();}
	if(! imagecopyresampled($imageResized, $img , 0 , 0 , 0 , 0 , $new_width , $new_height , $width , $height)){ echo "Resampling failed"; die(); }	
	if(! imagejpeg($imageResized , $dest)){ echo "Could not save new file"; die(); }	
