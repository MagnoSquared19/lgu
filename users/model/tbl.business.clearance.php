...
{	
	...
	{							
		...
		$db= new control(); 
		echo $db->j_select(['column'=>$col,'table'=>"doc_biz_clearance",'condition'=>$con,'sort'=>$sort,'paginate'=>$_POST['page']]);
	}
	...
	if(isset($gid))
	{
		...
		if($_POST['ft']=='0-1')
		{
			...
			$query = $db->db_insert("INSERT INTO doc_biz_clearance(`trans_no`,`resi_id`,`dated`,`remarks`,`d_type`,`bness_name`,`bness_street_address`,`bness_no`,`bness_industry`,`renew`,`bness_manager`,`price`) VALUES(
		...
		if($_POST['ft']=='0-2')
		{
			...
			$tbl= "doc_biz_clearance `R` LEFT JOIN residents `C` ON `R`.resi_id=`C`.id LEFT JOIN doc_settings `S` ON `S`.id='1' ";
			$q= $db->db_gen("SELECT `R`.id,`R`.d_type,`R`.remarks,`R`.remarks_admin,`R`.bness_name,`R`.bness_street_address,`R`.bness_no,`R`.bness_industry,`R`.renew,`R`.bness_manager,
			...
			{ 
				while($r=mysqli_fetch_array($q))
				{
					...
					array_push($rtn, array('id'=>$r['id'],'d_type'=>$r['d_type'],'remarks'=>$r['remarks'],'remarks_admin'=>$r['remarks_admin'],'bness_name'=>$r['bness_name'],'bness_address'=>$r['bness_street_address'],'bness_no'=>$r['bness_no'],'bness_type'=>$r['bness_industry'],'renew'=>$r['renew'],'bness_manager'=>$r['bness_manager'],
		...
		if($_POST['ft']=='0-3')
		{
			...
			$db = new control(); $db_con = $db->db_conn();			
			$query = $db->db_update("UPDATE `doc_biz_clearance` SET	
				...
				`bness_no`='".mysqli_real_escape_string($db_con, $fd['bness_no'])."',
				`bness_industry`='".mysqli_real_escape_string($db_con, $fd['bness_type'])."',
				`renew`='".mysqli_real_escape_string($db_con, $fd['renew'])."',
				`bness_manager`='".mysqli_real_escape_string($db_con, $fd['bness_manager'])."',
				`price`= (SELECT `biz_clearance_price` FROM doc_settings)
		...
		if($_POST['ft']=='0-4'){ $db= new control();  echo $db->j_delete("`doc_biz_clearance`",$_POST['newRow']); }		
	}	
}
