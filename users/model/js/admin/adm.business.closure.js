...
{
	$(".bzAdd_btn").click(function(e){	addNew_record();	});
	$(".bzEdt_btn").click(function(e){	getFrom_record();	});
	$(".bzDel_btn").click(function(e){	confirm_("Record will be permanently DELETED! Continue?","Confirmation", delFrom_record);	});
	...
	'<div class="pPanel_bg pop-bg"><form class="'+form+' pPanel popup-panel draggable" autocomplete="off">'+
		...
		'<div class="popPanel_wrap popup-panel-wrap">'+			
			'<div class="popup-panel-body"><div class="popup-panel-body-in">'+
				...
				'<div class="t-dvr"></div>'+
				'<div class="m-fr">'+
					'<div class="ipt-lfr"><i class="ipt-lbl">Business Name</i><input type="text" name="bness_name" required placeholder="Business Name" /></div>'+
					'<div class="ipt-rfr"><i class="ipt-lbl">Address <i class="ipt-lbl-notes">(street only)</i></i><input type="text" name="bness_address" required placeholder="Business Address" /></div>'+
				'</div>'+
				'<div class="m-fr">'+
					'<div class="ipt-lfr"><i class="ipt-lbl">Business Type</i><input type="text" name="bness_type" required placeholder="Business Type" /></div>'+
					'<div class="ipt-rfr">'+
						'<div class="l70-fr"><i class="ipt-lbl">Managed By</i><input type="text" name="bness_manager" required placeholder="Managed By" /></div>'+
						'<div class="r30-fr"><i class="ipt-lbl">Since</i><input type="number" name="bness_since" required placeholder="Year" min="1900" max="2021" step="1" /></div>'+						
					'</div>'+
				'</div>'+
				'<div class="m-fr">'	+
					'<div class="l30-fr"><i class="ipt-lbl">Order Type</i>'+
...
{	
	$.post('model/tbl.business.closure.php',{token:"0",fb:"27",ft:"0-1-2"},function(json, status, xhr)
	{
	...
	},"json").done(function(xhr, status, err)
	{
		...
		$(".aNR_form").unbind("submit").submit(function(e)
		{
			...
			$.post('model/tbl.business.closure.php',{token:"0",fb:"27",ft:"0-1",data_:inputData},function(json, status, xhr)
			{		
				$.each(json,function(i, v)
				{
					if(v.result == "1")
	...
	loading_(); generate_form("aNR_form","ADD BUSINESS CLOSURE","u");
}
...
{	
	...
	{	
		var tmpStat="", tmpProd="";
		$.post('model/tbl.business.closure.php',{token:"0",fb:"27",ft:"0-2",data_:gmi()},function(json, status, xhr)
		{
			$.each(json,function(i, v)
			{												
				if(v.id)
				{
					...
					$(".pPanel input[name='bness_manager']").val(v.managed_by);
					$(".pPanel input[name='bness_since']").val(v.since);
					$(".uPhoto_pic").attr("src",v.image);						
				}
			}); 
		},"json").done(function(xhr, status, err)
		{
			...
			$(".uNR_form").unbind("submit").submit(function(e)
			{
				...
				$.post('model/tbl.business.closure.php',{token:"0",fb:"27",ft:"0-3",data_:get_UserInfo($(this).serialize(),gd[0]['itoken'])},function(json, status, xhr)
				{		
					$.each(json,function(i, v)
					{
						if(v.result == "1")
		...
		loading_(); generate_form("uNR_form","UPDATE BUSINESS CLOSURE");
	}		
}
...
var set_delete = function(consID_arg)
{
	if($(".tblRowActive").length)
	{
		$.ajax({url:"model/tbl.business.closure.php",type:"post", data:{token:"0",fb:"27",ft:"0-4",newRow:gmi()},success:function(data){	loading_("unload");	if(data == "1"){ pLoader_("reload");dynamicFunc=function(){$(".bzEdt_btn, .bzDel_btn").attr("disabled",true);dynamicFunc="";};	alert_("Record was Successfully DELETED!","Information",dynamicFunc);}	else{alert_(data,"Information");}	}	});	loading_("Deleting, Please wait...");
	}
