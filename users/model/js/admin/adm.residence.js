...
{
	$.post('model/tbl.residence.php',{token:"0",fb:"27",ft:"0-1-2"},function(json, status, xhr)
	{
	...
	},"json").done(function(xhr, status, err)
	{
		...
		$(".aNR_form").unbind("submit").submit(function(e)
		{
			...
			$.post('model/tbl.residence.php',{token:"0",fb:"27",ft:"0-1",data_:inputData},function(json, status, xhr)
			{		
				$.each(json,function(i, v)
				{
					if(v.result == "1")
...
{	
	...
	{	
		var tmpStat="", tmpProd="";
		$.post('model/tbl.residence.php',{token:"0",fb:"27",ft:"0-2",data_:gmi()},function(json, status, xhr)
		{
			$.each(json,function(i, v)
			{												
				if(v.id)
		...
		},"json").done(function(xhr, status, err)
		{
			...
			$(".uNR_form").unbind("submit").submit(function(e)
			{
				...
				$.post('model/tbl.residence.php',{token:"0",fb:"27",ft:"0-3",data_:get_UserInfo($(this).serialize(),gd[0]['itoken'])},function(json, status, xhr)
				{		
					$.each(json,function(i, v)
					{
						if(v.result == "1")
...
var set_delete = function(consID_arg)
{
	if($(".tblRowActive").length)
	{
		$.ajax({url:"model/tbl.residence.php",type:"post", data:{token:"0",fb:"27",ft:"0-4",newRow:gmi()},success:function(data)
		{	
			loading_("unload");	
			if(data == "1")
			{ 
