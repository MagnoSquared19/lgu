...
{
	$(".patAdd_btn").click(function(e){	addNew_record();	});
	$(".patEdt_btn").click(function(e){	getFrom_record();	});
	$(".patDel_btn").click(function(e){	confirm_("Record will be permanently DELETED! Continue?","Confirmation", delFrom_record);	});
...
{	
	$.post('model/tbl.barangay.clearance.php',{token:"0",fb:"27",ft:"0-1-2"},function(json, status, xhr)
	{
	...
	},"json").done(function(xhr, status, err)
	{
		...
		$(".aNR_form").unbind("submit").submit(function(e)
		{
			...
			$.post('model/tbl.barangay.clearance.php',{token:"0",fb:"27",ft:"0-1",data_:inputData},function(json, status, xhr)
			{		
				$.each(json,function(i, v)
				{
					if(v.result == "1")
	...
	loading_(); generate_form("aNR_form","ADD BRGY. CLEARANCE","u");
}
...
{	
	...
	{	
		var tmpStat="", tmpProd="";
		$.post('model/tbl.barangay.clearance.php',{token:"0",fb:"27",ft:"0-2",data_:gmi()},function(json, status, xhr)
		{
			$.each(json,function(i, v)
			{												
				if(v.id)
		...
		},"json").done(function(xhr, status, err)
		{
			...
			$(".uNR_form").unbind("submit").submit(function(e)
			{
				...
				$.post('model/tbl.barangay.clearance.php',{token:"0",fb:"27",ft:"0-3",data_:get_UserInfo($(this).serialize(),gd[0]['itoken'])},function(json, status, xhr)
				{		
					$.each(json,function(i, v)
					{
						if(v.result == "1")
		...
		loading_(); generate_form("uNR_form","UPDATE BRGY. CLEARANCE");
	}		
}
...
var set_delete = function(consID_arg)
{
	if($(".tblRowActive").length)
	{
		$.ajax({url:"model/tbl.barangay.clearance.php",type:"post", data:{token:"0",fb:"27",ft:"0-4",newRow:gmi()},success:function(data){	loading_("unload");	if(data == "1"){ pLoader_("reload");dynamicFunc=function(){$(".patEdt_btn, .patDel_btn").attr("disabled",true);dynamicFunc="";};	alert_("Record was Successfully DELETED!","Information",dynamicFunc);}	else{alert_(data,"Information");}	}	});	loading_("Deleting, Please wait...");
	}
