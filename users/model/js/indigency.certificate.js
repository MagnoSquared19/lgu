...
{	
	...
	{	
		...
		$.post('model/tbl.indigency.certificate.php',{token:"0",fb:"27",ft:"0",uLike:plParent.find(".tblSearch").val(),sort:_sortFilter(plParent),page:_pagiFilter(plParent)},function(json, status, xhr)
		{		
			$.each(json,function(i, v)
			{
				...
				{	
					...
					'<div class="tblRow t-row" id="tblRow_'+v.id+'">'+
						...
						'</div>'+tmpNav_lbl+
						'<div class="t-colb">'+
							'<div class="t-col  ic-a"><div class="cTitle t-val"><b>Type: </b>'+tmpDType+'</div> </div>'+
							'<div class="t-col  ic-b"><div class="cTitle t-val"><b>Date Issued: </b>'+fromDbDate(v.date_issued)+'</div> </div>'+
							'<div class="t-col  ic-c"><div class="cTitle t-val txt-r"><b>Income: </b>'+toDecimal(v.income)+'</div> </div>'+
							'<div class="t-colz ic-d"><div class="cTitle t-val"><b>Remarks: </b>'+((v.remarks)?v.remarks+"<br/>":"")+((v.remarks_admin)?"Admin: "+v.remarks_admin:"")+'</div> </div>'+
						'</div>'+
					'</div>';
