...
if(isset($gid) && isset($_POST['token']) && $_POST['token'] == "0")
{
	...
	if(isset($_POST['ft']) && $_POST['ft'] == '0')
	{
		...
		$con= "(`P`.title LIKE '%".$_POST['uLike']."%' OR `P`.description LIKE '%".$_POST['uLike']."%') 
		AND `P`.inactive='0' $fid";
		$tbl= "about `P` LEFT JOIN users `U` ON `U`.id=`P`.user_id ";
		$q= $db->db_gen("SELECT `P`.id,`P`.user_id,`P`.title,`P`.description,IFNULL(`P`.dt_updated,`P`.dt_inserted) AS dated,
			...
			FROM $tbl WHERE $con GROUP BY `P`.id $desc ".$db->paginate($_POST['page'])." ;");
		...
		if(isset($_POST['ft']) && $_POST['ft'] == '0-0-1')
		{
			...
			$col= "id,title,description,dated";
			$que= "SELECT id,title,description,IFNULL(dt_updated, dt_inserted) AS dated FROM about";
			echo $db->j_select(['query'=>$que,'column'=>$col,'table'=>"about",'condition'=>$con]);
		}
	...
	if($glevel=="1" || $glevel=="2")
	{
		if(isset($_POST['ft']) && $_POST['ft'] == '0-1')
		{
			...
			$query= $db->db_insert_id("INSERT INTO `about`(`title`,`description`,`user_id`)	
				VALUES(
			...
			else
			{
				...
				if(isset($_FILES['data_']))
				{
					...
					if(!$fle_err[0]){ require_once('file.parser.php'); $tUpload = gUploader($nID,null,"../files/about/","data_",null, '900',"900"); }
				}					
		...
		{ 
			...
			$q= $db->db_gen("SELECT `P`.id,`P`.title,`P`.description FROM about `P` 
		...
		{				
			...
			$que= $db->db_update("UPDATE `about` SET 
			...
			{
				$fle_err= $_FILES['data_']['error'];
				if(!$fle_err[0]){ require_once('file.parser.php'); $tUpload= gUploader($id,null,"../files/about/","data_",null, '900',"900","false"); }
			}
			$files= $db->get_file_info("account/files/about/".$id,null,['array'=>true]);				
			
			array_push($rtn,['result'=>$que,'id'=>"$id",'title'=>$fd['title'],'description'=>html_entity_decode($_POST['desc']),'files'=>$files]);
			echo json_encode($rtn);
		}

		if(isset($_POST['ft']) && $_POST['ft'] == '0-4')  { $db= new control();  echo $db->j_delete("`about`",$_POST['newRow'],['json'=>true]); }
	}
