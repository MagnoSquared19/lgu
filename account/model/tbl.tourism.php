...
if(isset($gid) && isset($_POST['token']) && $_POST['token'] == "0")
{
	...
	if(isset($_POST['ft']) && $_POST['ft'] == '0')
	{
		...
		$tbl= "tourism `P` LEFT JOIN users `U` ON `U`.id=`P`.user_id ";
		...
		if(isset($_POST['ft']) && $_POST['ft'] == '0-0-1')
		{
			...
			echo $db->j_select(['column'=>"id,title",'table'=>"tourism",'condition'=>$con,'sort'=>"00-title"]);
		}
	...
	if($glevel=="1" || $glevel=="2")
	{
		if(isset($_POST['ft']) && $_POST['ft'] == '0-1')
		{
			...
			$query= $db->db_insert_id("INSERT INTO `tourism`(`title`,`description`,`user_id`)	
				VALUES(
			...
			else
			{
				...
				if(isset($_FILES['data_']))
				{
					...
					if(!$fle_err[0]){ require_once('file.parser.php'); $tUpload = gUploader($nID,null,"../files/tourism/","data_",null, '900',"900"); }
				}					
		...
		{ 
			...
			$q= $db->db_gen("SELECT `P`.id,`P`.title,`P`.description FROM tourism `P` 
		...
		{				
			...
			$que= $db->db_update("UPDATE `tourism` SET 
			...
			{
				$fle_err= $_FILES['data_']['error'];
				if(!$fle_err[0]){ require_once('file.parser.php'); $tUpload= gUploader($id,null,"../files/tourism/","data_",null, '900',"900","false"); }
			}
			$files= $db->get_file_info("account/files/tourism/".$id,null,['array'=>true]);				
		...
		if(isset($_POST['ft']) && $_POST['ft'] == '0-4')  { $db= new control();  echo $db->j_delete("`tourism`",$_POST['newRow'],['json'=>true]); }
	}
