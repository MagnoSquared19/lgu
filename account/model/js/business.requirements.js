...
var plParent=$("#pPoster"), pLoader_bool=true, tmpRstLoad= function(e){ plParent.attr("data-page","0"); plParent.find(".patPost_panel").empty(); pLoader_("filter",e); };
...
{	
	if(pLoader_bool)
	{
		...
		$.post('model/tbl.business.requirements.php',{token:"0",fb:"27",ft:"0",uLike:"",sort:tSort,page:plParent.attr("data-limit")+","+plParent.attr("data-page"),uLike:plParent.find(".pageSearch_ipt").val(),fid:((tmpID_arg)?tmpID_arg:""),ftrFr:$(".lFtrFr_ipt").val(),ftrTo:$(".lFtrTo_ipt").val(),ftr:removeString($(".lFtr_rad:checked").attr("id"))},function(json, status, xhr)
		{		
			$.each(json,function(i, v)
			{
				...
				{	
					...
					var tPostResult =''+
					'<div class="patPost pagelet-post" id="patPost'+v.id+'">'+
						'<div class="pagelet-header '+((v.me=="1")?'pagelet-post-adm':'')+'">'+
							'<div class="pagelet-poster">'+
								...
								'<div class="pagelet-poster-details">'+
									...
									'</div>'+
								'</div>'+
							'</div>'+((v.me=="1")?tPNavi("br",v.id):'')+	
						'</div>'+
						'<div class="pagelet-body-in" >'+
							'<div class="patPost_tit poster-post-title">'+v.title+'</div>'+
							'<pre class="patPost_val poster-post-body rich-paragraph">'+v.description+'</pre>'+
						...
						'</div>'+
					'</div>'; 
					plParent.find(".patPost_panel").append(tPostResult);
					plParent.find("#patPost"+v.id).data("j-v-c", {ci:v.itoken});
					jGallery_items(v.files,"#patPost"+v.id);
				}
				if(v.count)
				{ 
					if(v.count=="0"){ plParent.find(".patPost_panel").append('<div class="patResult_val pagelet-result">-- NO RECORD FOUND --</div>'); }
					else
					{
						var tRNum= $(".patPost").length, tRCnt=parseInt(v.count);
						if(tRNum == tRCnt)	{ plParent.find(".patPost_panel").append('<div class="patResult_val pagelet-result">-- END --</div>'); }
						else if(tRNum<tRCnt)
						{ 
							plParent.find(".patPost_panel").append('<div class="patPostShowMore_btn pagelet-show-more"><a href="#">SHOW MORE</a></div>');
							$(".patPostShowMore_btn a").unbind("click").click(function(e){ pLoader_(); e.preventDefault(); });
						}
					}
					$(".pageItem_cnt").html(toThousand(v.count)+" <p>Result"+((parseInt(v.count)>1)?"s":"")+"</p>");
					$(".patPostCount_wrap").text(toThousand(v.count)+" Result"+((parseInt(v.count)>1)?"s":""));
				}	
		...
		},"json").done(function(e)
		{
			...
			$(".brDel_btn").unbind("click").click(function(e){ var tmpPar=$(this), tmpRD= function(e){ delFrom_record(tmpPar);}; confirm_("Record will be permanently DELETED! Continue?","Confirmation", tmpRD); });
			$(".brEdt_btn").unbind("click").click(function(e){ getFrom_record($(this)); });
		...
		plParent.find(".patPost_panel").append(ldgPost); $(".patPostShowMore_btn, .acResult_inf").remove();
	}	
}

var func1_= function(e)
{
	$.post('model/tbl.business.requirements.php',{token:"0",fb:"27",ft:"0-0-1",uLike:""},function(json, status, xhr)
	{		
		$.each(json,function(i, v)
		{
			if(v.id)
