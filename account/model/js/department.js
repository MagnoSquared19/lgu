...
{	
	if(pLoader_bool)
	{
		...
		$.post('model/tbl.department.php',{token:"0",fb:"27",ft:"0",uLike:plParent.find(".tblSearch").val(),sort:_sortFilter(plParent),page:_pagiFilter(plParent)},function(json, status, xhr)
		{		
			$.each(json,function(i, v)
			{
				...
				{	
					...
					'<div class="tblRow t-row" id="tblRow_'+v.id+'">'+
						...
						'</div>'+tmpNav_lbl+
						'<div class="t-colb">'+
							'<div class="t-col  dep-a"><div class="cTitle t-val"			><b>Head: </b>'+v.head+'</div> </div>'+
							'<div class="t-col  dep-b"><div class="cTitle t-val break-word"	><b>Phone No.: </b>'+v.cp+'</div> </div>'+
							'<div class="t-colz dep-c"><div class="cTitle t-val"			><b>Description :</b>'+v.address+'</div> </div>'+
						'</div>'+
					'</div>';
