...
{
	$(".brAdd_btn").unbind("click").click(function(e){ addNew_record(); 			});
...
{	
	...
	'<div class="pPanel_bg pop-bg"><form class="'+form+' pPanel popup-panel draggable" autocomplete="off">'+
		...
		'<div class="popPanel_wrap popup-panel-wrap">'+
			'<div class="genMInputs_wrap popup-panel-body">'+
				...
				'<div class="patPoster_wrap rpt-update-img-wrap"></div>'+
	...
	mulFiles_(".pmPhoto_ipt",'<span class="mFiles_fr mf-g-photo" title="uplFile_name" > <img src="uplFile_src" /> <div class="mf-g-photo-in">	<div class="peFilesDel_btn mf-g-photo-del" data-file="uplFileData_name" title="Click to remove"><i class="fa fa-close"></i></div> </div> <p>uplFile_name</p> </span>',{parent:".patPoster_wrap",extension:false});
	//mulFiles_(".pmPhoto_ipt",'<span class="mFiles_fr mf-g-photo" title="uplFile_name" > <img src="uplFile_src" /> <div class="mf-g-photo-in">	<div class="peFilesDel_btn mf-g-photo-del" data-file="uplFile_name" title="Click to remove"><i class="fa fa-close"></i></div> </div> <p>uplFile_name</p> </span>',{parent:".patPoster_wrap",extension:false});
}

var addNew_record= function(e)
{
	generate_form("aNR_form","ADD REQUIREMENTS");show2_(".pPanel_bg", ".pPanel", {focus:((device() == "pc")?".pPanel input[name='title']":"")});
	$(".aNR_form").unbind("submit").submit(function(e)
	{
		e.preventDefault();	var iptData= new FormData(this); var data_= $(this).serialize();
		$.ajax({url: "model/tbl.business.requirements.php",type: "POST",data:iptData,dataType:"JSON",contentType:false, cache:false, processData:false,
		...
		success:function(json, status, xhr)
		{
			$.each(json, function(e, v)
			{
				if(v.result == "1")
				{
					plParent.attr("data-page","0"); $(".patPost_panel").empty(); pLoader_(); func1_();
...
{	
	if(e)
	{
		$.post('model/tbl.business.requirements.php',{token:"0",fb:"27",ft:"0-2",data_:e.parents(".patPost").data("j-v-c").ci},function(json, status, xhr)
		{
				...
				if(v.id)
				{
					...
					{						
						if(val.index)
						{
							...
							$(".patPoster_wrap").append('<span class="mFiles_fr mf-g-photo" title="'+val.name+'" > <img src="'+tPmSrc+'" /> <div class="mf-g-photo-in">	<div class="peFilesDel_btn mf-g-photo-del" data-file="'+val.src+'" title="Click to remove"><i class="fa fa-close"></i></div> </div> '+tPmLnk+' </span>'); //<p>'+tPmLnk+'</p>
						}
					});	
				}
			}); 
			...
			{	
				...
				success:function(json, status, xhr)
				{
					$.each(json, function(e, v)
					{
						if(v.result== "1")
						{
							$("#patPost"+v.id).find(".patPost_tit").text(v.title);
							$("#patPost"+v.id).find(".patPost_sti").text(v.sub_title);
							$("#patPost"+v.id).find(".patPost_dte p").text("On "+v.date_on);							
							$("#patPost"+v.id).find(".patPost_ven p").text(v.venue);
							$("#patPost"+v.id).find(".patPost_val").html(v.description);
							$("#patPost"+v.id).find(".PostPdf_wrap").empty().hide();
							$("#patPost"+v.id).find(".PostImage_wrap").hide().find(".Odeum").empty().removeClass("Odeum1, Odeum4");
							jGallery_items(v.files,"#patPost"+v.id); $(".Poster_photo, .Gallery_item").imgPreload(); $(".GalVideo_item").imgPreload();						
		...
		loading_();generate_form("uNR_form","UPDATE REQUIREMENTS");
	}		
}
...
{	
	if(e)
	{
		$.post('model/tbl.business.requirements.php',{token:"0",fb:"27",ft:"0-4",newRow:e.parents(".patPost").data("j-v-c").ci},function(json, status, xhr)
		{
			$.each(json,function(i, v)
			{												
				if(v.result=="1")
				{
					$("#patPost"+v.id).fadeOut(200,function(e)
					{ 
						$("#patPost"+v.id).remove();
						if($(".patPost").length < parseInt(plParent.attr("data-limit"))){ plParent.attr("data-page",(parseInt(plParent.attr("data-page")) - 1)); tmpRstLoad(); }
					});
					func1_();
