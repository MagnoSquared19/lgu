...
{
	$(".adAdd_btn").unbind("click").click(function(e) { addNew_record(); 			});
...
{	
	...
	'<div class="pPanel_bg pop-bg"><form class="'+form+' pPanel popup-panel draggable" autocomplete="off">'+
		...
		'<div class="popPanel_wrap popup-panel-wrap">'+
			'<div class="genMInputs_wrap popup-panel-body">'+
				...
				'<div class="adPoster_wrap rpt-update-img-wrap"></div>'+
	...
	$(".Richtext_wrap").jTextEditor({class:"adRichtext_ipt",placeholder:"Descripton"});
	
	$(".pmPhoto_btn").unbind("click").click(function(e){ e.preventDefault(); $(".pmPhoto_ipt").click(); });
	mulFiles_(".pmPhoto_ipt",'<span class="mFiles_fr mf-g-photo" title="uplFile_name" > <img src="uplFile_src" /> <div class="mf-g-photo-in">	<div class="peFilesDel_btn mf-g-photo-del" data-file="uplFileData_name" title="Click to remove"><i class="fa fa-close"></i></div> </div> <p>uplFile_name</p> </span>',{parent:".adPoster_wrap",extension:false});
}

var addNew_record= function(e)
{
	generate_form("aNR_form","ADD ABOUT BARANGAY");show2_(".pPanel_bg", ".pPanel", {focus:((device() == "pc")?".pPanel input[name='title']":"")});
	$(".aNR_form").unbind("submit").submit(function(e)
	{
		e.preventDefault();	var iptData= new FormData(this); var data_= $(this).serialize();
		$.ajax({url: "model/tbl.about.barangay.php",type: "POST",data:iptData,dataType:"JSON",contentType:false, cache:false, processData:false,
		beforeSend:function(data,form,opt){	loading_(loading_progress,"<i></i>");	iptData.append("token","0");iptData.append("fb","27");iptData.append("ft","0-1");iptData.append("desc",$(".adRichtext_ipt").html());iptData.append("data_",data_); for(var i=0, len=mFiles.length;i<len;i++){ iptData.append('data_[]', mFiles[i]);}  },
		success:function(json, status, xhr)
		{
			$.each(json, function(e, v)
			{
				if(v.result == "1")
				{
					plParent.attr("data-page","0"); $(".adPost_panel").empty(); pLoader_(); func1_();
...
{	
	if(e)
	{
		$.post('model/tbl.about.barangay.php',{token:"0",fb:"27",ft:"0-2",data_:e.parents(".adPost").data("j-v-c").ci},function(json, status, xhr)
		{
			...
			{												
				if(v.id)
				{
					...
					$(".adRichtext_ipt").html(v.description);
					...
					{						
						if(val.index)
						{
							...
							$(".adPoster_wrap").append('<span class="mFiles_fr mf-g-photo" title="'+val.name+'" > <img src="'+tPmSrc+'" /> <div class="mf-g-photo-in">	<div class="peFilesDel_btn mf-g-photo-del" data-file="'+val.src+'" title="Click to remove"><i class="fa fa-close"></i></div> </div> '+tPmLnk+' </span>'); //<p>'+tPmLnk+'</p>
						}
					});	
				}
			}); 
			...
			{	
				...
				success:function(json, status, xhr)
				{
					$.each(json, function(e, v)
					{
						if(v.result== "1")
						{
							$("#adPost"+v.id).find(".adPost_tit").text(v.title);
							$("#adPost"+v.id).find(".adPost_val").html(v.description);
							$("#adPost"+v.id).find(".PostPdf_wrap").empty().hide();
							$("#adPost"+v.id).find(".PostImage_wrap").hide().find(".Odeum").empty().removeClass("Odeum1, Odeum4");
							jGallery_items(v.files,"#adPost"+v.id); $(".Poster_photo, .Gallery_item").imgPreload(); $(".GalVideo_item").imgPreload();						
		...
		loading_();generate_form("uNR_form","UPDATE ABOUT BARANGAY");
	}		
}
...
{	
	if(e)
	{
		$.post('model/tbl.about.barangay.php',{token:"0",fb:"27",ft:"0-4",newRow:e.parents(".adPost").data("j-v-c").ci},function(json, status, xhr)
		{
			$.each(json,function(i, v)
			{												
				if(v.result=="1")
				{
					$("#adPost"+v.id).fadeOut(200,function(e)
					{ 
						$("#adPost"+v.id).remove();
						if($(".adPost").length < parseInt(plParent.attr("data-limit"))){ plParent.attr("data-page",(parseInt(plParent.attr("data-page")) - 1)); tmpRstLoad(); }
					});
					func1_();
