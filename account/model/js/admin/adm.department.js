...
{
	$(".depAdd_btn").click(function(e){	addNew_record("new");	});
	$(".depEdt_btn").click(function(e){	getFrom_record();	});
	$(".depDel_btn").click(function(e){	confirm_("Record will be permanently DELETED! Continue?","Confirmation", delFrom_record);	});
...
{	
	...
	'<div class="pPanel_bg pop-bg"><form class="'+form+' pPanel popup-panel draggable" autocomplete="off">'+
		...
		'<div class="popPanel_wrap popup-panel-wrap">'+			
			'<div class="genMInputs_wrap popup-panel-body">'+
				'<div class="m-fr"><i class="ipt-lbl">Office Department</i><input type="text" name="name" required placeholder="Office / Department" /></div>'+
				'<div class="m-fr">'+
					'<div class="ipt-lfr"><i class="ipt-lbl">Head</i>	  <input type="text" name="head" placeholder="Department Head" /></div>'+
					'<div class="ipt-rfr"><i class="ipt-lbl">Phone No.</i><input type="text" name="phone" placeholder="Phone Number" /></div>'+
				'</div>'+
	...
	var gPanel= function(form,hdr)
	{
		var form_ipt= function(o)
		{
			...
			var tmpGPanelIpt= ''+
			'<div class="ipt-lfr">'+
				'<div class="ipt-lfr"> <i class="ipt-lbl">Department</i><input type="text" name="name" required placeholder="Office / Department" /> </div>'+
				'<div class="ipt-rfr"> <i class="ipt-lbl">Head</i> <input type="text" name="head" placeholder="Department Head" /> </div>'+
			'</div>'+
			'<div class="ipt-rfr">'+
				'<div class="l30-fr"> <i class="ipt-lbl">Phone No.</i> <input type="text" name="phone" placeholder="Phone Number" /> </div>'+
				'<div class="r70-fr"> <i class="ipt-lbl">Office Address</i> <input type="text" name="address" placeholder="Office Address" /> </div>'+
			'</div>';
			return tmpGPanelIpt;
		};		
...
{
	if(a){ generate_form("aNR_form","ADD NEW DEPARTMENT","u");show2_(".pPanel_bg", ".pPanel", {focus:((device() == "pc")?".pPanel input[name='qty']":"")}); }			
	$(".aNR_form").unbind("submit").submit(function(e)
	{
		e.preventDefault(); var inputData= InputSerialize($(this));
		$.post('model/tbl.department.php',{token:"0",fb:"27",ft:"0-1",forms_:inputData},function(json, status, xhr)
		{		
...
{	
	...
	{	
		var tmpStat="", tmpProd="";
		$.post('model/tbl.department.php',{token:"0",fb:"27",ft:"0-2",data_:gmi()},function(json, status, xhr)
		{
			$.each(json,function(i, v)
			{												
				if(v.id)
				{
					...
					$(".pPanel input[name='head']").val(v.head);
		...
		{
			loading_("unload"), show2_(".pPanel_bg", ".pPanel", {focus:((device() == "pc")?".pPanel input[name='qty']":"")});
			$(".uNR_form").unbind("submit").submit(function(e)
			{
				e.preventDefault(); var gd=eval(err.responseText);	
				$.post('model/tbl.department.php',{token:"0",fb:"27",ft:"0-3",data_:get_UserInfo($(this).serialize(),gd[0]['itoken'])},function(json, status, xhr)
				{		
					$.each(json,function(i, v)
					{
						if(v.result == "1")
		...
		loading_(); generate_form("uNR_form","UPDATE DEPARTMENT");
	}		
}
...
var set_delete = function(consID_arg)
{
	if($(".tblRowActive").length)
	{
		$.ajax({url:"model/tbl.department.php",type:"post", data:{token:"0",fb:"27",ft:"0-4",newRow:gmi()},success:function(data){	loading_("unload");	if(data == "1"){	dynamicFunc=function(){pLoader_("reload");$(".depEdt_btn, .depDel_btn").attr("disabled",true);dynamicFunc="";};	alert_("Record was Successfully DELETED!","Information",dynamicFunc);}	else{alert_(data,"Information");}	}	});	loading_("Deleting, Please wait...");
	}
