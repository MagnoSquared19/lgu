...
{
	$(".albumAdd_btn").unbind("click").click(function(e){ addNew_record(); 			});
...
{	
	...
	'<div class="pPanel_bg pop-bg"><form class="'+form+' pPanel popup-panel draggable" autocomplete="off">'+
		...
		'<div class="popPanel_wrap popup-panel-wrap">'+
			'<div class="genMInputs_wrap popup-panel-body">'+
				'<div class="m-fr"><i class="ipt-lbl">Album Name</i><input type="text" name="title" required placeholder="Album Name" /></div>'+
				'<div class="gallery-upload-wrap"> <div class="albumPoster_wrap rpt-update-img-wrap"></div> </div>'+
	...
	mulFiles_(".pmPhoto_ipt",'<span class="mFiles_fr mf-g-photo" title="uplFile_name" > <img src="uplFile_src" /> <div class="mf-g-photo-in">	<div class="peFilesDel_btn mf-g-photo-del" data-file="uplFileData_name" title="Click to remove"><i class="fa fa-close"></i></div> </div> <p>uplFile_name</p> </span>',{parent:".albumPoster_wrap",extension:false});
}

var addNew_record= function(e)
{
	generate_form("aNR_form","ADD ALBUM");show2_(".pPanel_bg", ".pPanel", {focus:((device() == "pc")?".pPanel input[name='title']":"")});
	$(".aNR_form").unbind("submit").submit(function(e)
	{
		e.preventDefault();	var iptData= new FormData(this); var data_= $(this).serialize();
		$.ajax({url: "model/tbl.album.php",type: "POST",data:iptData,dataType:"JSON",contentType:false, cache:false, processData:false,
		...
		success:function(json, status, xhr)
		{
			$.each(json, function(e, v)
			{
				if(v.result == "1")
				{
					plParent.attr("data-page","0"); $(".albumPost_panel").empty(); pLoader_(); func1_();
...
{	
	if(e)
	{
		$.post('model/tbl.album.php',{token:"0",fb:"27",ft:"0-2",data_:e.parents(".albumPost").data("j-v-c").ci},function(json, status, xhr)
		{
				...
				if(v.id)
				{
					...
					{						
						if(val.index)
						{
							...
							$(".albumPoster_wrap").append('<span class="mFiles_fr mf-g-photo" title="'+val.name+'" > <img src="'+tPmSrc+'" /> <div class="mf-g-photo-in">	<div class="peFilesDel_btn mf-g-photo-del" data-file="'+val.src+'" title="Click to remove"><i class="fa fa-close"></i></div> </div> '+tPmLnk+' </span>'); //<p>'+tPmLnk+'</p>
						}
					});	
				}
			}); 
			...
			{	
				...
				success:function(json, status, xhr)
				{
					$.each(json, function(e, v)
					{
						if(v.result== "1")
						{
							$("#albumPost"+v.id).find(".albumPost_tit").text(v.title);
							$("#albumPost"+v.id).find(".PostImage_wrap").hide().find(".Odeum").empty().removeClass("Odeum1, Odeum4");
							jGallery_items(v.files,"#albumPost"+v.id); $(".Poster_photo, .Gallery_item").imgPreload(); $(".GalVideo_item").imgPreload();	

							$("#albumPost"+v.id).find(".PostNoAlbum_txt").remove(); 
							if(v.files.length <= 1){ $("#albumPost"+v.id).find(".pageGallery_body").append('<div class="PostNoAlbum_txt album-no-file">No File Found</div>'); }					
		...
		loading_();generate_form("uNR_form","UPDATE ALBUM");
	}		
}
...
{	
	if(e)
	{
		$.post('model/tbl.album.php',{token:"0",fb:"27",ft:"0-4",newRow:e.parents(".albumPost").data("j-v-c").ci},function(json, status, xhr)
		{
			$.each(json,function(i, v)
			{												
				if(v.result=="1")
				{
					$("#albumPost"+v.id).fadeOut(200,function(e)
					{ 
						$("#albumPost"+v.id).remove();
						if($(".albumPost").length < parseInt(plParent.attr("data-limit"))){ plParent.attr("data-page",(parseInt(plParent.attr("data-page")) - 1)); tmpRstLoad(); }
					});
					func1_();
