...
{	
	...
	'<div class="pPanel_bg pop-bg"><form class="'+form+' pPanel popup-panel draggable" autocomplete="off">'+
		...
		'<div class="popPanel_wrap popup-panel-wrap">'+
			'<div class="genMInputs_wrap popup-panel-body">'+
				...
				'<div class="faqPoster_wrap rpt-update-img-wrap"></div>'+
	...
	mulFiles_(".pmPhoto_ipt",'<span class="mFiles_fr mf-g-photo" title="uplFile_name" > <img src="uplFile_src" /> <div class="mf-g-photo-in">	<div class="peFilesDel_btn mf-g-photo-del" data-file="uplFileData_name" title="Click to remove"><i class="fa fa-close"></i></div> </div> <p>uplFile_name</p> </span>',{parent:".faqPoster_wrap",extension:false});
	//mulFiles_(".pmPhoto_ipt",'<span class="mFiles_fr mf-g-photo" title="uplFile_name" > <img src="uplFile_src" /> <div class="mf-g-photo-in">	<div class="peFilesDel_btn mf-g-photo-del" data-file="uplFile_name" title="Click to remove"><i class="fa fa-close"></i></div> </div> <p>uplFile_name</p> </span>',{parent:".faqPoster_wrap",extension:false});
}

var addNew_record= function(e)
{
	generate_form("aNR_form","ADD POSTER");show2_(".pPanel_bg", ".pPanel", {focus:((device() == "pc")?".pPanel input[name='title']":"")});
	$(".aNR_form").unbind("submit").submit(function(e)
	{
		e.preventDefault();	var iptData= new FormData(this); var data_= $(this).serialize();
		$.ajax({url: "model/tbl.faq.php",type: "POST",data:iptData,dataType:"JSON",contentType:false, cache:false, processData:false,
		...
		success:function(json, status, xhr)
		{
			$.each(json, function(e, v)
			{
				if(v.result == "1")
				{
					plParent.attr("data-page","0"); $(".faqPost_panel").empty(); pLoader_(); func1_();
...
{	
	if(e)
	{
		$.post('model/tbl.faq.php',{token:"0",fb:"27",ft:"0-2",data_:e.parents(".faqPost").data("j-v-c").ci},function(json, status, xhr)
		{
				...
				if(v.id)
				{
					...
					{						
						if(val.index)
						{
							...
							$(".faqPoster_wrap").append('<span class="mFiles_fr mf-g-photo" title="'+val.name+'" > <img src="'+tPmSrc+'" /> <div class="mf-g-photo-in">	<div class="peFilesDel_btn mf-g-photo-del" data-file="'+val.src+'" title="Click to remove"><i class="fa fa-close"></i></div> </div> '+tPmLnk+' </span>'); //<p>'+tPmLnk+'</p>
						}
					});	
				}
			}); 
			...
			$(".uNR_form").unbind("submit").submit(function(s)
			{
				...
				success:function(json, status, xhr)
				{
					$.each(json, function(e, v)
					{
						if(v.result== "1")
						{
							$("#faqPost"+v.id).find(".faqPost_tit").text(v.title);
							$("#faqPost"+v.id).find(".faqPost_val").html(v.description);
							$("#faqPost"+v.id).find(".PostPdf_wrap").empty();
							$("#faqPost"+v.id).find(".PostImage_wrap").hide().find(".Odeum").empty().removeClass("Odeum1 Odeum4");
		...
		loading_();generate_form("uNR_form","UPDATE PROJECT");
	}		
}
...
{	
	if(e)
	{
		$.post('model/tbl.faq.php',{token:"0",fb:"27",ft:"0-4",newRow:e.parents(".faqPost").data("j-v-c").ci},function(json, status, xhr)
		{
			$.each(json,function(i, v)
			{												
				if(v.result=="1")
				{
					$("#faqPost"+v.id).fadeOut(200,function(e)
					{ 
						$("#faqPost"+v.id).remove();
						if($(".faqPost").length < parseInt(plParent.attr("data-limit"))){ plParent.attr("data-page",(parseInt(plParent.attr("data-page")) - 1)); tmpRstLoad(); }
					});
					func1_();
