...
{
	$(".notifAdd_btn").unbind("click").click(function(e){ addNew_record(); 				});
...
{	
	...
	'<div class="pPanel_bg pop-bg"><form class="'+form+' pPanel popup-panel draggable" autocomplete="off">'+
		...
		'<div class="popPanel_wrap popup-panel-wrap">'+
			'<div class="genMInputs_wrap popup-panel-body">'+
				'<div class="l80-fr"><i class="ipt-lbl">Title</i><input type="text" name="title" required placeholder="Title" /></div>'+
				...
				'</div>'+
				'<div class="t-dvr"></div>'+
				'<div class="TextEditor_wrap m-fr"><i class="ipt-lbl">Descripton</i></div>'+
				'<div class="notifPoster_wrap rpt-update-img-wrap"></div>'+
				'<input type="file" name="uplFile[]" class="pmPhoto_ipt" hidden accept="image/*,video/*,application/pdf" multiple capture="camera" />'+
			'</div>'+
	...
	$(".TextEditor_wrap").jTextEditor({height:"250px",class:"notifDescription_txt",placeholder:"Description"});

	$(".pmPhoto_btn").unbind("click").click(function(e){ e.preventDefault(); $(".pmPhoto_ipt").click(); });
	mulFiles_(".pmPhoto_ipt",'<span class="mFiles_fr mf-g-photo" title="uplFile_name" > <img src="uplFile_src" /> <div class="mf-g-photo-in">	<div class="peFilesDel_btn mf-g-photo-del" data-file="uplFile_name" title="Click to remove"><i class="fa fa-close"></i></div> </div> <p>uplFile_name</p> </span>',{parent:".notifPoster_wrap",extension:false});
}

var addNew_record= function(e)
{
	generate_form("aNR_form","ADD NOTIFICATION");show2_(".pPanel_bg", ".pPanel", {focus:((device() == "pc")?".pPanel input[name='title']":"")});
	$(".aNR_form").unbind("submit").submit(function(e)
	{
		e.preventDefault();	var iptData= new FormData(this); var data_= $(this).serialize();
		$.ajax({url: "model/tbl.notifications.php",type: "POST",data:iptData,dataType:"JSON",contentType:false, cache:false, processData:false,
		beforeSend:function(data,form,opt){	loading_("Saving, please wait...");	iptData.append("token","0");iptData.append("desc",$(".notifDescription_txt").html());iptData.append("fb","27");iptData.append("ft","0-1");iptData.append("data_",data_); for(var i=0, len=mFiles.length;i<len;i++){ iptData.append('data_[]', mFiles[i]);}  },
		success:function(json, status, xhr)
		{
			$.each(json, function(e, v)
			{
				if(v.result == "1")
				{
					plParent.attr("data-page","0"); $(".notifPost_panel").empty(); pLoader_(); 
...
{	
	if(e)
	{
		$.post('model/tbl.notifications.php',{token:"0",fb:"27",ft:"0-2",data_:e.parents(".notifPost").data("j-v-c").ci},function(json, status, xhr)
		{
			...
			{												
				if(v.id)
				{
					...
					$(".notifDescription_txt").html(v.description);
					...
					{						
						if(val.index)
						{
							...
							$(".notifPoster_wrap").append('<span class="mFiles_fr mf-g-photo" title="'+val.name+'" > <img src="'+tPmSrc+'" /> <div class="mf-g-photo-in">	<div class="peFilesDel_btn mf-g-photo-del" data-file="'+val.src+'" title="Click to remove"><i class="fa fa-close"></i></div> </div> '+tPmLnk+' </span>'); //<p>'+tPmLnk+'</p>
						}
					});	
				}
			}); 
			...
			$(".uNR_form").unbind("submit").submit(function(s)
			{
				...
				success:function(json, status, xhr)
				{
					$.each(json, function(e, v)
					{
						if(v.result== "1")
						{
							$("#notifPost"+v.id).find(".notifPost_tit").text(v.title);
							$("#notifPost"+v.id).find(".notifPost_view").html('<p><b>Viewer:</b> '+v.viewer+'</p>');
							$("#notifPost"+v.id).find(".notifPost_val").html(v.description);
							$("#notifPost"+v.id).find(".PostPdf_wrap").empty().hide();
							$("#notifPost"+v.id).find(".PostImage_wrap").hide().find(".Odeum").empty().removeClass("Odeum1, Odeum4");
							
							jGallery_items(v.files,"#notifPost"+v.id); $(".GalImage_item").imgPreload(); $(".GalVideo_item").imgPreload();							
							alert_("Record was Successfully Updated.", "Information"); $(".pPanel_bg").remove();							
						}
						else{alert_("Error in : "+v.result,"Error Message");} 							
					});	
		...
		loading_();generate_form("uNR_form","UPDATE NOTIFICATION");
	}		
}
...
{	
	if(e)
	{
		$.post('model/tbl.notifications.php',{token:"0",fb:"27",ft:"0-4",newRow:e.parents(".notifPost").data("j-v-c").ci},function(json, status, xhr)
		{
			$.each(json,function(i, v)
			{												
				if(v.result=="1")
				{
					$("#notifPost"+v.id).fadeOut(200,function(e)
					{ 
						$("#notifPost"+v.id).remove();
						if($(".notifPost").length < parseInt(plParent.attr("data-limit"))){ plParent.attr("data-page",(parseInt(plParent.attr("data-page")) - 1)); tmpRstLoad(); }
					});
