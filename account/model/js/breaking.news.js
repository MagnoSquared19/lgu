...
{	
	if(pLoader_bool)
	{
		...
		$.post('model/tbl.breaking.news.php',{token:"0",fb:"27",ft:"0",uLike:plParent.find(".tblSearch").val(),sort:_sortFilter(plParent),page:_pagiFilter(plParent)},function(json, status, xhr)
		{		
					...
					'<div class="tblRow t-row" id="tblRow_'+v.id+'">'+
						...
						'<div class="tColName t-cola '+tmpNav_class+'" >'+
							...
							'<div class="t-col-name">'+
								'<label class="cTitle t-val" for="t-row'+v.id+'"><p>'+v.title+'</p><i class="fa fa-angle-down"></i></label>'+
							'</div>'+
						'</div>'+tmpNav_lbl+
						'<div class="t-colb">'+
							'<div class="t-col  bn-a" ><div class="cTitle t-val"><b>Date: </b>'+fromDbDate(v.dated)+'</div> </div>'+
							'<div class="t-colz bn-b" ><div class="cTitle t-val"><b>Description  </b>'+v.description+'</div> </div>'+
						'</div>'+
					'</div>';
