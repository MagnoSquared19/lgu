...
{	
	if(pLoader_bool)
	{
		...
		$.post('model/tbl.bid.supplement.php',{token:"0",fb:"27",ft:"0",uLike:plParent.find(".tblSearch").val(),sort:_sortFilter(plParent),page:_pagiFilter(plParent),dFr:plParent.find(".tblFr_ipt").val(),dTo:plParent.find(".tblTo_ipt").val()},function(json, status, xhr)
		{		
			$.each(json,function(i, v)
			{
				...
				{	
					...
					'<div class="tblRow t-row" id="tblRow_'+v.id+'">'+
						...
						'</div>'+tmpNav_lbl+
						'<div class="t-colb">'+
							'<div class="t-col  bs-a"><div class="cTitle t-val c"><b>Bulletin No.: </b>'+v.bulletin_no+'</div> </div>'+
							'<div class="t-col  bs-b"><div class="cTitle t-val break-word"><b>Date: </b>'+fromDbDate(v.dated)+'</div> </div>'+
							'<div class="t-col  bs-c"><div class="cTitle t-val"><b>Particulars: </b>'+v.particulars+'</div> </div>'+
							'<div class="t-colz bs-d"><div class="cTitle t-val"><b>Changes: </b>'+v.changes+'</div> </div>'+
						'</div>'+
					'</div>';
