...
{	
	...
	if($glevel=="1" || $glevel=="2")
	{
		if($_POST['ft'] == '0-1')
		{
			...
			{
				$q= $db->db_insert("INSERT INTO directory(`name`,`cp`,`mail`,`address`) VALUES(
					'".mysqli_real_escape_string($db_con, $tr[$i]["name"])."',
					'".mysqli_real_escape_string($db_con, $tr[$i]["phone"])."',
					'".mysqli_real_escape_string($db_con, $tr[$i]["mail"])."',
		...
		if($_POST['ft'] == '0-2')
		{
			$db= new control(); echo $db->j_select(['table'=>"directory",'condition'=>"id='".$db->itoken($_POST['data_'])."'"]);
		}			
		...
		if($_POST['ft'] == '0-4'){ $db= new control();  echo $db->j_delete("`directory`",$_POST['newRow']); }			
	}	
}
